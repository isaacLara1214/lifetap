...
import { getTotalDamageFromOpponent, getTotalDamageFromCommander } from '../../features/session/utils'
      ...
      {expanded && (
        <div className="mt-3 space-y-3 max-h-[280px] overflow-y-auto">
          {opponents.map((opponent) => {
            const totalDamage = getTotalDamageFromOpponent(player, opponent.id)
              ...
              <div key={opponent.id} className="bg-black/20 rounded-lg p-3">
                <div className="flex items-center justify-between mb-2">
                  ...
                  <span className={`text-sm font-bold ${
                    totalDamage >= 21 ? 'text-red-400' : 
                    totalDamage >= 15 ? 'text-orange-400' : 
                    'text-white/80'
                  }`}>
                    {totalDamage}/21
                  </span>
                  ...
                  {opponent.commanders.map((commander) => {
                    ...
                    return (
                      <div key={commander.id} className="flex items-center gap-2">
                        ...
                        <span className={`text-xs font-bold min-w-[24px] text-center ${
